...
import { ApproForm } from '../components/approvisionnements/ApproForm';
import { ApproTable } from '../components/approvisionnements/ApproTable';
import type { Approvisionnement } from '../types';

const ITEMS_PER_PAGE = 20;

export function Approvisionnements() {
  ...
  const loadData = async (page = 1, date = selectedDate) => {
    ...
    try {
      ...
      const [approsRes, countRes, servicesRes] = await Promise.all([
        supabase
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-slate-600">Chargement des approvisionnements...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Approvisionnements</h1>
          <p className="text-slate-600">Gestion des entrées et sorties de fonds</p>
        </div>
        {canManage && (
          <button
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-sm text-slate-600">
            {totalCount} approvisionnement{totalCount > 1 ? 's' : ''}
          </span>
        </div>
      </div>

      <ApproTable approvisionnements={approvisionnements} loading={loading} />
