/*
  # Correction du Sens du Cash pour les Transactions Mixtes

  ## Description
  Cette migration corrige la direction des flux de cash dans toutes les fonctions
  de transactions mixtes. Le cash doit suivre ces règles:
  
  - **RETRAIT** : Le cash DIMINUE (on paie le client)
  - **DÉPÔT** : Le cash AUGMENTE (on reçoit du client)

  ## Problème identifié
  Les fonctions actuelles inversaient ces règles:
  - Retrait augmentait le cash ❌
  - Dépôt diminuait le cash ❌
  ...
  ### Service virtuel = montant principal
  - **Retrait** → + montant principal (augmente le solde virtuel)
  - **Dépôt** → - montant principal (diminue le solde virtuel)

  ### Cash réel = montant payé/reçu
  - **Retrait** → - montant payé au client (cash diminue)
  - **Dépôt** → + montant reçu du client (cash augmente)

  ## Exemples
  
  ### Retrait 100 USD, payé 60 USD + 88,000 CDF (taux 2,200)
  - Service virtuel USD : -100 USD
  - Cash USD : -60 USD (payé au client)
  - Cash CDF : -88,000 CDF (payé au client)

  ### Dépôt 250,000 CDF, reçu 150,000 CDF + 100 USD (taux 2,500)
  - Service virtuel CDF : +250,000 CDF
  - Cash CDF : +150,000 CDF (reçu du client)
  - Cash USD : +100 USD (reçu du client)
*/
...
BEGIN
  ...
  -- CORRECTION: Retrait diminue le solde virtuel
  UPDATE services
  SET
  ...
  -- CORRECTION: Retrait diminue le cash (on paie le client)
...
BEGIN
  ...
  -- CORRECTION: Dépôt augmente le solde virtuel
  UPDATE services
  SET
    solde_virtuel_usd = solde_virtuel_usd + p_montant_total_usd,
    updated_at = now()
  ...
  -- CORRECTION: Dépôt augmente le cash (on reçoit du client)
  UPDATE global_balances
  SET
...
COMMENT ON FUNCTION create_transaction_mixte_retrait IS 'Crée une transaction de retrait avec montant total en USD et paiement mixte USD/CDF - CASH DIMINUE';
COMMENT ON FUNCTION create_transaction_mixte_depot IS 'Crée une transaction de dépôt avec montant total en USD et réception mixte USD/CDF - CASH AUGMENTE';
COMMENT ON FUNCTION create_transaction_mixte_retrait_cdf IS 'Crée une transaction de retrait avec montant total en CDF et paiement mixte CDF/USD - CASH DIMINUE';
COMMENT ON FUNCTION create_transaction_mixte_depot_cdf IS 'Crée une transaction de dépôt avec montant total en CDF et réception mixte CDF/USD - CASH AUGMENTE';
