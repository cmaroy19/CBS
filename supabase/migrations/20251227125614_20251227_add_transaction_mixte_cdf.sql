/*
  # Fonctions de Transaction Mixte avec Montant Total en CDF

  ## Description
  Cette migration ajoute des fonctions pour gérer les transactions avec paiement mixte
  lorsque le montant total est exprimé en CDF (et non en USD).
  ...
  Retrait de 128000 CDF payé avec 100000 CDF + 12.73 USD (taux 2200):
  ```sql
  SELECT create_transaction_mixte_retrait_cdf(
    p_service_id := 'uuid-du-service',
    p_montant_total_cdf := 128000,
    p_montant_paye_cdf := 100000,
    p_montant_paye_usd := 12.73,
