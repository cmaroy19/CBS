/*
  # Correction Retrait USD avec Lignes de Conversion

  ## Problème
  La fonction create_transaction_mixte_retrait (retraits USD) ne crée pas
  de lignes de conversion pour équilibrer la comptabilité double entrée.

  ## Solution
  Ajouter les lignes de conversion marquées avec is_conversion_line = true
  pour équilibrer les débits et crédits dans chaque devise.

  ## Exemple
  Retrait de 59 USD payé avec 50 USD + 20,700 CDF (taux: 1 USD = 2,300 CDF)
  
  **Lignes créées :**
  1. Débit virtuel USD : +59 USD (créance)
  2. Crédit cash USD : -50 USD (paiement)
  3. Crédit virtuel USD : -9 USD (conversion) - IGNORÉE PAR TRIGGER
  4. Débit virtuel CDF : +20,700 CDF (conversion) - IGNORÉE PAR TRIGGER
  5. Crédit cash CDF : -20,700 CDF (paiement)
  6. Crédit virtuel CDF : -20,700 CDF (conversion) - IGNORÉE PAR TRIGGER
  
  **Résultat :**
  - Virtuel USD : +59 (lignes 1 seulement, 3 ignorée)
  - Virtuel CDF : 0 (lignes 4 et 6 s'annulent, toutes deux ignorées)
  - Cash USD : -50
  - Cash CDF : -20,700
*/
...
BEGIN
  ...
  IF p_montant_paye_cdf > 0 THEN
    ...
    -- Ligne 6: Crédit virtuel CDF (conversion - pour équilibrer) - IGNORÉE PAR TRIGGER
    INSERT INTO transaction_lines (
      header_id,
      ligne_numero,
      type_portefeuille,
      ...
      'credit',
      p_montant_paye_cdf,
      'Crédit service virtuel CDF (annulation conversion)',
      true
    );
  END IF;
...
BEGIN
  ...
  IF p_montant_recu_cdf > 0 THEN
    ...
    -- Ligne 5: Débit virtuel CDF (conversion - pour équilibrer) - IGNORÉE PAR TRIGGER
    INSERT INTO transaction_lines (
      header_id,
      ligne_numero,
      type_portefeuille,
      ...
      'debit',
      p_montant_recu_cdf,
      'Débit service virtuel CDF (annulation conversion)',
      true
    );
    v_ligne_numero := v_ligne_numero + 1;
  END IF;
...
COMMENT ON FUNCTION create_transaction_mixte_retrait IS 'Retrait USD mixte - Les soldes sont mis à jour automatiquement par le trigger. Les lignes de conversion USD/CDF servent uniquement à équilibrer la comptabilité double entrée.';
COMMENT ON FUNCTION create_transaction_mixte_depot IS 'Dépôt USD mixte - Les soldes sont mis à jour automatiquement par le trigger. Les lignes de conversion USD/CDF servent uniquement à équilibrer la comptabilité double entrée.';
